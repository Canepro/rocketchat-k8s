...
The weekly maintenance automation uses the repo runner plus Codex/Grafana MCP checks:
...
See `runbooks/weekly-aks-maintenance.md` for the stop conditions. In short: public GitHub issue/PR updates and repo-backed GitOps changes are allowed on Vincent's personal repos when evidence supports them and Azure or OKE reconciles from Git; secret-value handling, out-of-band live mutation, Azure cost actions, ingress changes, RBAC changes, Terraform apply, direct Helm upgrades, Argo CD sync/prune, and auto-shutdown changes still need explicit approval.
