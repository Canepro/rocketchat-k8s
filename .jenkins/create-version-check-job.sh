#!/bin/bash
# Script to create the version-check Jenkins job
# This creates a scheduled Pipeline job (not multibranch) that runs daily
# Usage: bash create-version-check-job.sh [repo-owner] [repo-name] [job-name]
...
JOB_NAME="${3:-version-check-${REPO_NAME}}"
JENKINS_URL="${JENKINS_URL:-https://jenkins.canepro.me}"
CONFIG_FILE="${CONFIG_FILE:-.jenkins/version-check-job-config.xml}"
...
TEMP_CONFIG=$(mktemp -t version-check-config.XXXXXX.xml)
...
if [ "$HTTP_CODE" = "200" ] || [ "$HTTP_CODE" = "201" ]; then
  ...
  echo ""
  echo "The job is scheduled to run weekdays at 5 PM (after cluster starts at 4 PM)."
